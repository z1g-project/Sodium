...
const getActiveFrameId = () => {
    if (document.querySelector('.chrome-tab[active]'))
        return (
            +document.querySelector('.chrome-tab[active]').getAttribute('ifd') +
            1
        )
    return null
}
...
function getIcon(id) {
    ...
    if (urlIco !== null) {
        ...
        return (
            ...
            path +
            xor.encode(
                'http://' + CONTENT_WINDOW(id).document.domain + '/favicon.ico',
            )
        )
}
...
function setInfo(frameId) {
    ...
    URL_BAR.value = ''
    if (!CONTENT_WINDOW(frameId).location.href.includes(path)) {
    ...
    if (
        CONTENT_WINDOW(frameId).document.getElementsByTagName('title')[0]
        ...
        document.getElementsByClassName(frameId)[0].firstChild.data =
            xor.decode(regUrl.split(path).slice(1).join(path))
    //set url bar
    if (getActiveFrameId() == frameId) {
        URL_BAR.value = xor.decode(regUrl.split(path).slice(1).join(path))
    }
    // set the favicon of page
    document.querySelector(
        `div[ifd="${+frameId - 1}"]`,
    ).children[2].children[0].attributes[1].value =
        `background-image: url(${getIcon(frameId)})`
    if (
        document
...
function toggleId(...x) {
    ...
    x.forEach((frame) => {
        if (
            getComputedStyle(document.getElementById(frame)).display === 'none'
        ) {
            showId(frame)
        } else {
            hideId(frame)
...
document.querySelector('#urlbar').addEventListener('keydown', (event) => {
    if (event.key !== 'Enter') {
        ...
        try {
            fetch(
                prefix +
                    xor.encode(`https://duckduckgo.com/ac?q=${URL_BAR.value}`),
            )
                .then((a) => a.text())
                .then(
                    (a) => JSON.parse(`[${a.split('\n')[5].slice(0, -1)}]`)[0],
                )
                .then((a) => {
    ...
    if (
        ...
        value =
            '//' + location.host + path + xor.encode('http://' + URL_BAR.value)
        document.getElementById(getActiveFrameId()).src = value
    } else {
        value = '//' + location.host + path + xor.encode(URL_BAR.value)
        document.getElementById(getActiveFrameId()).src = value
    }
...
function applyTheme(frame) {
    switch (frame) {
        case 'Apply':
            ...
            localStorage.setItem(
                'mockb',
                document.getElementById('tabact').value,
            )
            location.reload()
            break
        case 'Dark':
...
function AddBookmark(id) {
    ...
    console.log(
        CONTENT_WINDOW(id).location.href +
            ...
            getIcon(id) +
            ' ' +
            CONTENT_WINDOW(id).document.getElementsByTagName('title')[0]
                .firstChild.textContent,
    )
    data.push([
        CONTENT_WINDOW(id).location.href,
